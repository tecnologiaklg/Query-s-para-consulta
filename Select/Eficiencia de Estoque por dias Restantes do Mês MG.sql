...
(SELECT
MMG.CODPROD,
(MMG.QTD_MG / DIA.DM)*(DIA.DM - DIA.DIA) AS QTD_DIA,
MMG.QTD_MG,
...
/*QTD ESTQ MIN MG*/
    ...
    TGFPRO PRO
    WHERE
    PRO.AD_QTDMIN_MNG IS NOT NULL
    AND PRO.AD_QTDMIN_MNG <> 0) MMG
LEFT JOIN
/*ESTOQUE ATUAL*/
    (SELECT
    EST.CODPROD,
    ...
    AND EST.CODEMP IN ('3')
    GROUP BY
    EST.CODPROD) EST ON EST.CODPROD = MMG.CODPROD,
/*QTD DIAS NO MÊS*/
    (SELECT
    ...
    AD_CALENDARIO CAL
    WHERE
    TO_CHAR(CAL.DATACALEN,'mm/yyyy') = TO_CHAR(CURRENT_DATE,'mm/yyyy')) DIA) AAA